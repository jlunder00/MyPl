...
enum TokenType {
  ...
  // reserved words
  TYPE, WHILE, FOR, TO, DO, IF, THEN, ELSEIF, ELSE, END, FUN, VAR, RETURN, NEW, IMPORT, FROM, AS,
  // primitive types
...
private:
  ...
  std::map<TokenType,std::string> token_type_map =
    { // basic symbols
      ...
      // reserved words
      {TYPE, "TYPE"}, {WHILE, "WHILE"}, {FOR, "FOR"},
      {TO, "TO"}, {DO, "DO"}, {IF, "IF"}, {THEN, "THEN"},
      {ELSEIF, "ELSEIF"}, {ELSE, "ELSE"}, {END, "END"}, 
      {FUN, "FUN"}, {VAR, "VAR"}, {RETURN, "RETURN"}, {NEW, "NEW"},
