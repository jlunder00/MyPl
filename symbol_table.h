...
public:
  ...
  std::list<std::string> get_namespaces() const;
  
  void add_namespace(std::string name, std::string fname);
...
private:
  ...
  //typedef std::map<std::string, std::map<std::string,SymTableObject*>> Environment;
  ...
  std::map<std::string, std::string> namespaces_to_filenames;
...
SymbolTable::~SymbolTable()
{
  ...
  /*
  for (std::pair<int,Environment> p1 : environments) {
    for (std::pair<std::string, std::pair<std:string,SymTableObject*>> p2 : p1.second)
    {
      for (std::pair<std::string,SymTableObject*> p3 : p2.second)
        delete_sym_obj(p3.second);
      p2.second.clear();
    }
    p1.second.clear()
  }
...
void SymbolTable::add_namespace(const std::string name, const std::string fname)
{
  if(!has_namespace(name))
  {
    namespaces.push_back(name);
    namespaces_to_filenames.insert(std::pair<std::string, std::string>(name, fname));
  }
}
...
bool SymbolTable::has_namespace(std::string name)
{
...
std::list<std::string> SymbolTable::get_namespaces() const
{
  return namespaces;
}
